$(document).ready(() => {
    ...
    const f1 = (par) => {
        fetch(`/terminarz2${document.location.search}`).then((res) => {
            ...
            if (document.referrer !== `http://${document.location.host}/rozgrywki.html${document.location.search}`) {
                $(button).hide();
            }
            let licznik2 = 1;
            const div = [];
            ...
            const f2 = (par) => {
                fetch(`/terminarz4${document.location.search}`).then((res) => {
                    return res.json();
                }).then((data2) => {
                    if (data2) {
                        for (let l1 = 1; l1 <= teamArr.length; l1++) {
                            for (let l2 = 1; l2 <= teamArr.length; l2++) {
                                if (l1 !== l2) {
            ...
            if (par === 1) {
                ...
                while (licznik2 <= data[0].teams.length - 1) {
                    let licznik = 0;
                    while (licznik < data[0].teams.length / 2) {
                        if (data[0][`kolejka${licznik2}`][licznik].indexOf("pauza") === -1) {
    ...
    $(button).click(() => {
        f1(1);
        $(button).hide();
        fetch(`/terminarz5${document.location.search}`, {
            method: 'POST'
        })
    })
    fetch(`/terminarz5${document.location.search}`).then((res) => {
        return res.json();
    }).then((data) => {
        if (data === 1) {
    ...
    fetch(`/tabela${document.location.search}`).then((res) => {
        return res.json();
    }).then((data) => {
        if (data) {
            const tr = [];
